...
public class NewsTitleTextView extends TextView {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (isVerticalLine) {
            ...
            canvas.drawRect(0, 25, 1 * screen_density, this.getHeight() - 25, paint);
        }
        if (isHorizontaline) {
            paint = new Paint();
            paint.setColor(horizontalineColor);
            paint.setStyle(Style.FILL);
            canvas.drawRect(ViewUtils.dip2px(context, 6), getHeight() - 3, ViewUtils.dip2px(context, getWidth() * screen_density * 0.9f), getHeight() * screen_density, paint);
        }
