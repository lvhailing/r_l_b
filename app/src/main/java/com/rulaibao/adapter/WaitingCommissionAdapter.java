...
public class WaitingCommissionAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private final MouldList<WaitingCommissionList2B> list;
    Context mContext;
    ...
    public WaitingCommissionAdapter(Context context, MouldList<WaitingCommissionList2B> list) {
        mContext = context;
        this.list = list;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_ITEM) { // 加载待发佣金 item 布局
            View itemView = mInflater.inflate(R.layout.item_commission_detail, parent, false);
...
//                mContext.startActivity(intent);
    ...
    public void AddHeaderItem(MouldList<WaitingCommissionList2B> items) {
        list.addAll(0, items);
        notifyDataSetChanged();
    }

    public void AddFooterItem(MouldList<WaitingCommissionList2B> items) {
        list.addAll(items);
        notifyDataSetChanged();
    }
